...
For development, the CirclePack source files can be run from an IDE using SplashMain (./src/allMains/SplashMain). Why? I think the RunCirclePack main is needed to load the C libraries which seem to be already available after adding the above /jar subfolder (?).
